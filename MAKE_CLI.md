# 📦 Junie Prompt — Embed `includes/` and add `make` subcommand to **basic**
...
2. Add a CLI subcommand `make <target>` with behavior:
  ...
  * `basilc make examples/hello.bas` → write *just that file* into `./examples/hello.bas` then **run it** (as if `basilc run examples/hello.bas`).
  * `basilc make upgrade` → write `./upgrade.bas` from root of `includes/` then **run it**.
