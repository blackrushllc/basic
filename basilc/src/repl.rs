...
pub fn start_repl(mut sess: Session, maybe_path: Option<String>) {
    ...
    let mut last_file: String = "_.bas".to_string();
    ...
    loop {
        ...
        if io::stdin().read_line(&mut l).is_err() { break; }
        let line = l;
        ...
        if buffer.is_empty() {
            ...
            if upper == "LIST" {
                ...
                if parts.len() >= 2 {
                    ...
                    // Save program_buf to last_file (default _.bas) with blank-line gaps, then run
